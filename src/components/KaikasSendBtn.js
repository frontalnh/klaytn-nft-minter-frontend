...
import { setKaikasAddressRoutine, kaikasSendTxRoutine } from "../modules/kaikas";
...
const KaikasSendBtn = (props) => {
  ...
  const sendKaikas = async () => {
    ...
    const data = caver.klay.abi.encodeFunctionCall(
      ...
      [from, to, caver.utils.toBN(tokenId).toString()]
    );
    const txData = {
  ...
  return (
    <Button disabled={props.disabled} onClick={sendKaikas}>
      Kaikas로 보내기
    </Button>
  );
};
